<template>
  <div class="p-4 md:p-8 pb-20 max-w-7xl mx-auto print:p-0 print:pb-0 print:max-w-none">
    ...
    <!-- 커스텀 다이얼로그 -->
    <div v-if="dialog.show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm print:hidden">
...
const doSearch = async () => {
  ...
  try {
    ...
    console.error(err)
    await showAlert('조회 오류', '조회 중 오류가 발생했습니다.\n권한 또는 네트워크를 확인하세요.', 'error')
  } finally {
    loading.value = false
  }
}
